function output = floater_func(f, v)
...
next = sparse([f(:,1);f(:,1)],[f(:,2);f(:,3)],[c(:,1)./l(:,3);c(:,1)./l(:,2)],np,np); 
next = next+sparse([f(:,2);f(:,2)],[f(:,1);f(:,3)],[c(:,2)./l(:,3);c(:,2)./l(:,1)],np,np); 
next = next+sparse([f(:,3);f(:,3)],[f(:,1);f(:,2)],[c(:,3)./l(:,2);c(:,3)./l(:,1)],np,np); 
